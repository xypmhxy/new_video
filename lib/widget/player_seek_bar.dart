...
class PlayerSeekBar extends LeafRenderObjectWidget {
  ...
  const PlayerSeekBar({
    ...
    this.thumbColor,
    this.insideThumbRadius = 10.0,
    this.insideThumbColor,
    this.thumbGlowColor,
  ...
  final double insideThumbRadius;
  ...
  /// wrap [PlayerSeekBar] with a `Padding` widget if your layout needs to leave
  ...
  RenderObject createRenderObject(BuildContext context) {
    ...
    return _RenderProgressBar(
      ...
      insideThumbRadius: insideThumbRadius,
      insideThumbColor: insideThumbColor ?? primaryColor,
...
class _RenderProgressBar extends RenderBox {
  _RenderProgressBar({
    ...
    required double insideThumbRadius,
    required Color insideThumbColor,
    ...
    double timeLabelPadding = 0.0,
        ...
        _insideThumbRadius = insideThumbRadius,
        _insideThumbColor = insideThumbColor,
  ...
  void _onDragStart(DragStartDetails details) {
    ...
    onDragStart?.call(ThumbDragDetails(
      timeStamp: Duration(milliseconds: _currentThumbDuration()),
  ...
  void _onDragEnd(DragEndDetails details) {
    onDragEnd?.call();
    onSeek?.call(Duration(milliseconds: _currentThumbDuration()));
    _finishDrag();
  }
  ...
  int _currentThumbDuration() {
    final thumbMilliseconds = _thumbValue * total;
    return thumbMilliseconds.round();
  }
  ...
  Color get insideThumbColor => _insideThumbColor;
  Color _insideThumbColor;

  set insideThumbColor(Color value) {
    if (_insideThumbColor == value) return;
    _insideThumbColor = value;
    markNeedsPaint();
  }
  ...
  double get insideThumbRadius => _insideThumbRadius;
  double _insideThumbRadius;
  set insideThumbRadius(double value) {
    if (_insideThumbRadius == value) return;
    _insideThumbRadius = value;
    markNeedsLayout();
  }
  ...
  double _heightWhenNoLabels() {
    return max(2 * _thumbRadius, _barHeight) + 12;
  }
  ...
  void _drawBar(
      {required Canvas canvas, required Size availableSize, required double widthProportion, required Color color}) {
    ...
    final startPoint = Offset(capRadius, availableSize.height);
    var endPoint = Offset(dx, availableSize.height);
  ...
  void _drawThumb(Canvas canvas, Size localSize) {
    ...
    final center = Offset(thumbDx, localSize.height);
    if (_userIsDraggingThumb) {
      final thumbGlowPaint = Paint()..color = thumbGlowColor;
      canvas.drawCircle(center, thumbGlowRadius, thumbGlowPaint);
    }
    canvas.drawCircle(center, thumbRadius, thumbPaint);
    thumbPaint.color = insideThumbColor;
    canvas.drawCircle(center, insideThumbRadius, thumbPaint);
  }

  double _proportionOfTotal(int duration) {
    if (total == 0 || duration.isNegative) {
      return 0.0;
    }
